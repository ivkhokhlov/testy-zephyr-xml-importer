...
from .parser import iter_test_cases, parse_folders
...
def dry_run_import(
    ...
    *,
    ...
    xml_bytes = _read_source_bytes(xml_source)
    folders = parse_folders(xml_bytes)
    zip_index = _build_zip_index(attachments_zip)
    duplicate_key_counts = build_duplicate_key_counts(iter_test_cases(xml_bytes))
    ...
    for tc in iter_test_cases(xml_bytes):
        case_count += 1
        ...
        case_warnings = build_case_warnings(tc, payload["name"], duplicate_key_counts)
                ...
                if existing_case_id and on_duplicate == "skip":
                    ...
                    if existing_case_id and on_duplicate == "upsert":
                        try:
                            ...
                            action = "skipped"
                            case_id = existing_case_id
                            row_warnings.append("Upsert requested but adapter does not support updates")
                    else:
